...
object D_SaferFoldingUsingEval{
  ...
  def trivialNonStackSafeFold[A,B](list:List[A],acc:B)(f:(A,B)=>B):B={
    list match {
      case ::(head, next) => f(head,trivialNonStackSafeFold(next,f(head,acc))(f))
      case Nil => acc
    }
  }
  //stack-safe fold
  def stackSafeFold[A,B](list:List[A],acc:B)(f:(A,B)=>B):B={
    def stackSafeFoldEval(list:List[A],acc:B)(f:(A,B)=>B):Eval[B]= {
      list match {
        case ::(head, next) => Eval.defer(stackSafeFoldEval(next,f(head,acc))(f))
