...
class E_SaferFoldingUsingEval extends AnyFlatSpec with should.Matchers {
  ...
  "Trivial tail-recursive fold" should "not blow up the stack" in {
    ...
    catchStackOverflowError(() =>
      trivialFold(list, 0)(_ + _)
    ).toEither shouldBe Right(list.sum)
  }
  "Trivial non-tail-recursive fold" should "blow up the stack" in {
    val list = (0 to 50000).toList //fails
    catchStackOverflowError(() =>
      trivialNonStackSafeFold(list, 0)(_ + _)
    ).isSuccess shouldBe false
  }
  "Eval fold" should "not blow up the stack" in {
    val list = (0 to 50000).toList //fails
    catchStackOverflowError(() =>
      stackSafeFold(list, 0)(_ + _)
    ).toEither shouldBe Right(list.sum)
  }
}
