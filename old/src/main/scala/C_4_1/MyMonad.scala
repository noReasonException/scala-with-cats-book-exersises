...
object MonadOps {
  implicit class MyOptionMonadOps[A](src: MyOption[A]) {
    def flatMap[B](fn: A => MyOption[B])(implicit
        monad: MyMonad[MyOption]
    ): MyOption[B] = monad.flatMap(src)(fn)
    def map[B](fn: A => B)(implicit monad: MyMonad[MyOption]): MyOption[B] =
      monad.map(src)(fn)
