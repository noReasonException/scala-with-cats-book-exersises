...
object Instances{
  ...
  implicit def boxCodec[A](implicit boxedCodec:Codec[A]):Codec[Box[A]] = new Codec[Box[A]] {
    override def encode(element: Box[A]): String = boxedCodec.encode(element.element)

    override def decode(element: String): Box[A] = Box(boxedCodec.decode(element))
  }
}


/**
 * Note that the decode method of our Codec type class doesn’t account for failures.
 * If we want to model more sophisticated relationships we can move beyond functors
