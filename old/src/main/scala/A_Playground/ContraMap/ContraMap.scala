...
object Instances {
  ...
  /** * I cannot define globally the contraMap, because i need more information
    * about the underlying operation of a F[A-B]
    */
  implicit val printableContraMap: ContraMapAble[Printable] =
    new ContraMapAble[Printable] {
      override def contraMap[A, B](
          src: Printable[B]
      )(fn: A => B): Printable[A] = new Printable[A] {
...
object SyntaxOps {
  implicit class ContraMapSyntaxOps[F[_], B](element: F[B]) {
    def contraMap[A](fn: A => B)(implicit contra: ContraMapAble[F]): F[A] =
      contra.contraMap(element)(fn)
  }
}
